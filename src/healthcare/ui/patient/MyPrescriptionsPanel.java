...
public class MyPrescriptionsPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        String[] columns = {"ID", "Doctor", "Disease", "Date", "Notes"};
        ...
        styleTable(table, new Color(243, 156, 18));

        JScrollPane scroll = new JScrollPane(table);
        scroll.setBorder(BorderFactory.createLineBorder(new Color(200, 210, 220), 1));
        add(scroll, BorderLayout.CENTER);

        JButton refreshBtn = makeButton("REFRESH", new Color(243, 156, 18));
    ...
    private void loadData() {
        ...
        List<Prescription> list = service.getForPatient(patient.getPatientId());
        for (Prescription p : list) {
            tableModel.addRow(new Object[]{
                p.getPrescriptionId(), p.getDoctorName(), p.getDiseaseName(),
                p.getCreatedAt(), p.getDiagnosisNotes()
            });
        }
    }
}