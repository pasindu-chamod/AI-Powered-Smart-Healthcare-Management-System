...
public class ViewAIPredictionsPanel extends JPanel {

    private final AIPredictionDAO predictionDAO = new AIPredictionDAOImpl();
    private JTable table;
    private DefaultTableModel tableModel;

    public ViewAIPredictionsPanel() {
        buildUI();
        loadData();
    }
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("AI PREDICTIONS");
        title.setFont(new Font("SansSerif", Font.BOLD, 20));
        title.setForeground(Color.WHITE);
        titlePanel.add(title, BorderLayout.WEST);
        add(titlePanel, BorderLayout.NORTH);

        String[] columns = {"ID", "Patient ID", "Disease", "Confidence", "Date", "Reviewed"};
        ...
        JButton refreshBtn = makeButton("REFRESH", new Color(155, 89, 182));
        JButton reviewBtn = makeButton("MARK REVIEWED", new Color(46, 204, 113));
    ...
    private void loadData() {
        ...
        for (AIPrediction p : list) {
            tableModel.addRow(new Object[]{
                p.getPredictionId(), p.getPatientId(), p.getPredictedDisease(),
                String.format("%.1f%%", p.getConfidenceScore()),
                p.getCreatedAt(), p.isDoctorReviewed() ? "Yes" : "No"
            });
        }
    }
