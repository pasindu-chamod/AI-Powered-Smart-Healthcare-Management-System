...
public class AIPredictionsLogPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("AI PREDICTIONS LOG");
        title.setFont(new Font("SansSerif", Font.BOLD, 20));
        title.setForeground(Color.WHITE);
        titlePanel.add(title, BorderLayout.WEST);
        add(titlePanel, BorderLayout.NORTH);

        String[] columns = {"ID", "Patient ID", "Symptoms", "Disease", "Confidence", "Reviewed", "Date"};
        ...
        styleTable(table, new Color(44, 62, 80));

        add(new JScrollPane(table), BorderLayout.CENTER);

        JButton refreshBtn = makeButton("REFRESH", new Color(44, 62, 80));
        JButton markBtn = makeButton("MARK REVIEWED", new Color(46, 204, 113));
        refreshBtn.addActionListener(e -> loadData());
        markBtn.addActionListener(e -> markReviewed());
    ...
    private void loadData() {
        ...
        List<AIPrediction> list = predictionDAO.getAllPredictions();
        for (AIPrediction p : list) {
            String symptoms = p.getSymptomsEntered();
            if (symptoms != null && symptoms.length() > 35) symptoms = symptoms.substring(0, 35) + "...";

            tableModel.addRow(new Object[]{
                p.getPredictionId(), p.getPatientId(), symptoms,
                p.getPredictedDisease(), String.format("%.1f%%", p.getConfidenceScore()),
                p.isDoctorReviewed() ? "Yes" : "No", p.getCreatedAt()
            });
        }
    }
    ...
    private void markReviewed() {
        ...
        if (row < 0) { JOptionPane.showMessageDialog(this, "Select a prediction!"); return; }
        predictionDAO.markReviewed((int) tableModel.getValueAt(row, 0));
        JOptionPane.showMessageDialog(this, "Marked as reviewed!");
        loadData();
    }
}