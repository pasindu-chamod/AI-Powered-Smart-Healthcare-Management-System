...
public class ManagePatientsPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("MANAGE PATIENTS");
        title.setFont(new Font("SansSerif", Font.BOLD, 20));
        title.setForeground(Color.WHITE);
        titlePanel.add(title, BorderLayout.WEST);
        add(titlePanel, BorderLayout.NORTH);

        String[] columns = {"ID", "Full Name", "Email", "Phone", "Blood Group", "Gender", "DOB"};
        ...
        JButton refreshBtn = makeButton("REFRESH", new Color(46, 204, 113));
    ...
    private void loadData() {
        ...
        for (Patient p : list) {
            tableModel.addRow(new Object[]{
                p.getPatientId(), p.getFullName(), p.getEmail(),
                p.getPhone(), p.getBloodGroup(), p.getGender(), p.getDateOfBirth()
            });
        }
    }
}